...
BLINK_INTERVAL_SECONDS = 0.5 # Blink every 0.5 seconds
...
def demonstration(parking_lot_name: str = "default", image_path: str = None, initial_mode: str = None):
    ...
    global LAST_BLINK_CHANGE # To manage blinking timing
    ...
    print("- 'e': **Enter Edit ID Mode (E)** (then Left click inside a spot)")
    ...
    while True:
        ...
        if coordinate_generator.mode == 'p':
            ...
            mode_text = "Edit ID (E) - Click spot!"  # Edit mode
        elif coordinate_generator.mode == 'c':  
            mode_text = f"CALIBRATION (C) - Point {len(coordinate_generator.irregular_points)}/2"
            
        # Change message if text is being entered
        ...
        if coordinate_generator.is_editing_id:
             # If editing, keys are handled by _handle_text_input function
             if coordinate_generator._handle_text_input(key):
                 # Reset blinking time to maintain continuity after entering a character
                 LAST_BLINK_CHANGE = time.time() 
             
        # If not editing, handle mode-changing keys
        else:
             if key == ord("q") or key == 27:
                 ...
                 # Reset blinking state when entering edit mode
